...
import { ImageWrapper } from "pages/UserList/userlist.Style";
...
const EditDatabaseModal: React.FC = () => {
    ...
    return searchText
      ? allEventRegistees.filter((x) => {
        return (
          ...
          x?.jobTitle?.toLowerCase()?.includes(searchText.toLowerCase()) ||
  ...
  const columns = [
    ...
    },
    {
      ...
      Cell: ({ row }: any) => {
        return (
          <>{isEmpty(row.original.jobTitle) ? "--" : row.original.jobTitle}</>
        );
      },
    ...
    },
    {
      ...
      Cell: ({ row }: any) => {
        return (
          <>
            {isEmpty(row.original.companyName)
              ? "--"
        ...
        return (
          <>
            {actionSelected.index !== row.original.obj_id ? (
              <Fragment>
                {isEmpty(row.original.email) ? "--" : row.original.email}
              </Fragment>
            ) : (
              <Fragment>
                <CustomInput
    ...
    },
    {
      ...
      Cell: ({ row }: any) => {
        return (
          <>{(row.original.groups && row.original.groups.join(",")) || "--"}</>
        );
      },
      toolTip: ""
    },
